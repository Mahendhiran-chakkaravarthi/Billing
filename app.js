...
const defaults = {
  ...
  companies: [{ id: "company-1", name: "TBD Books", gstin: "", phone: "", address: { line1: "", line2: "", city: "", state: "Tamil Nadu", pincode: "" }, bank: { name: "", accountHolder: "", accountNumber: "", ifscCode: "" }, terms: "Thanks for doing business with us." }],
  parties: [], invoices: [], quotes: [], transactions: [],
  settings: { invoicePrefix: "INV", quotePrefix: "EST", currency: "Rs.", supportPhone: "", supportEmail: "", bankName: "", accountNumber: "", ifscCode: "" }
};
...
let pendingResetOtp = "";
let pendingResetMember = "";
function saveMembers() { localStorage.setItem("tbd-members", JSON.stringify(members)); }
...
function renderSession() { const member = activeMember(); $("#loginOverlay").classList.toggle("hidden", Boolean(member)); $("#sessionUser").textContent = member ? member.name : ""; $("#logoutButton").classList.toggle("hidden", !member); }
...
function renderSettings() { $("#invoicePrefix").value = state.settings.invoicePrefix; $("#quotePrefix").value = state.settings.quotePrefix; $("#currency").value = state.settings.currency; $("#supportPhone").value = state.settings.supportPhone || ""; $("#supportEmail").value = state.settings.supportEmail || ""; }
...
function showDocument(type, number) {
  ...
  if (signatureBox) signatureBox.textContent = "";
  ...
  if (details) details.insertAdjacentHTML("beforeend", `${entry.poNumber ? `<p>PO No.: ${entry.poNumber}</p>` : ""}${entry.campaignDate ? `<p>Campaign Date: ${entry.campaignDate}</p>` : ""}`);
...
$("#settingsForm").addEventListener("submit", (event) => { event.preventDefault(); state.settings = { invoicePrefix: $("#invoicePrefix").value.trim() || "INV", quotePrefix: $("#quotePrefix").value.trim() || "EST", currency: $("#currency").value.trim() || "Rs.", supportPhone: $("#supportPhone").value.trim(), supportEmail: $("#supportEmail").value.trim() }; save(); renderAll(); toast("General settings saved."); });
...
$("#shareDocument").addEventListener("click", async () => { const title = $("#printDocumentContent .reference-doc-title")?.textContent || "Billing Document"; const company = activeCompany().name; const message = `${company} - ${title} is ready.`; if ($("#shareFormat").value === "email") { window.location.href = `mailto:?subject=${encodeURIComponent(`${company} ${title}`)}&body=${encodeURIComponent(message)}`; return; } const file = new File([downloadWord(false)], documentFilename("doc"), { type: "application/msword" }); if (navigator.canShare?.({ files: [file] })) { try { await navigator.share({ title: `${company} ${title}`, text: message, files: [file] }); } catch {} } else { downloadWord(); window.open(`https://wa.me/?text=${encodeURIComponent(`${message} Document downloaded - please attach it in WhatsApp.`)}`, "_blank", "noopener"); } });
$("#loginForm").addEventListener("submit", (event) => { event.preventDefault(); const key = "admin"; if ($("#loginPassword").value !== members[key].password) return toast("Incorrect password."); localStorage.setItem("tbd-active-member", key); $("#loginPassword").value = ""; renderSession(); toast(`Welcome, ${members[key].name}.`); });
$("#logoutButton").addEventListener("click", () => { localStorage.removeItem("tbd-active-member"); renderSession(); });
$("#forgotPasswordButton").addEventListener("click", () => { $("#loginForm").classList.add("hidden"); $("#forgotPasswordForm").classList.remove("hidden"); });
$("#backToLoginButton").addEventListener("click", () => { $("#forgotPasswordForm").classList.add("hidden"); $("#loginForm").classList.remove("hidden"); });
$("#forgotMethod").addEventListener("change", () => { const email = $("#forgotMethod").value === "email"; $("#forgotIdentifierLabel").firstChild.textContent = email ? "Email ID" : "Mobile Number"; $("#forgotIdentifier").type = email ? "email" : "tel"; $("#forgotIdentifier").value = ""; });
$("#forgotPasswordForm").addEventListener("submit", (event) => { event.preventDefault(); const method = $("#forgotMethod").value; const savedValue = method === "email" ? state.settings.supportEmail : state.settings.supportPhone; const enteredValue = $("#forgotIdentifier").value.trim(); if (!savedValue) return toast("Add reset contact in App Settings first."); if (enteredValue.toLowerCase() !== savedValue.toLowerCase()) return toast("Mobile number or email does not match App Settings."); pendingResetOtp = "123456"; pendingResetMember = "admin"; $("#otpDeliveryText").textContent = `Enter the 6-digit OTP sent to ${enteredValue}.`; $("#forgotPasswordForm").classList.add("hidden"); $("#verifyOtpForm").classList.remove("hidden"); toast("OTP sent."); });
$("#verifyOtpForm").addEventListener("submit", (event) => { event.preventDefault(); if ($("#resetOtp").value !== pendingResetOtp) return toast("Invalid OTP."); $("#verifyOtpForm").classList.add("hidden"); $("#newPasswordForm").classList.remove("hidden"); $("#newPassword").focus(); });
$("#backToForgotButton").addEventListener("click", () => { $("#verifyOtpForm").classList.add("hidden"); $("#forgotPasswordForm").classList.remove("hidden"); });
$("#backToOtpButton").addEventListener("click", () => { $("#newPasswordForm").classList.add("hidden"); $("#verifyOtpForm").classList.remove("hidden"); });
$("#newPasswordForm").addEventListener("submit", (event) => { event.preventDefault(); const password = $("#newPassword").value; if (password !== $("#confirmPassword").value) return toast("Passwords do not match."); if (password.length < 6) return toast("Password needs at least 6 characters."); members[pendingResetMember].password = password; saveMembers(); pendingResetOtp = ""; $("#forgotPasswordForm").reset(); $("#verifyOtpForm").reset(); $("#newPasswordForm").reset(); $("#newPasswordForm").classList.add("hidden"); $("#loginForm").classList.remove("hidden"); toast("Password reset successful. Login with new password."); });
